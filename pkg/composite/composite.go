...
//TODO: Clear background with black or white for each image
//TODO: Have a working directory

// To4x6x3 composites the source images to a 6x4 format, with 3 frames per image. Each frame will
// get 4x2 in dimension
func To4x6x3(inputDir, outputDir string, verLog *log.Logger) error {
	if verLog == nil {
	...
	for i, img := range images {
		...
		//TODO: Handle other aspect ratio
		//TODO: Want minimum amount of space on side, not blocked by flipping
		sourceWidth := 1280.0
		sourceHeight := 720.0
		targetHeight := 600.0

		minY := int(float64(i%imagesPerSheet) * targetHeight)
		maxX := int(targetHeight / sourceHeight * sourceWidth)
